...
Controls (in the OpenCV window):
    Left-click in cam0 pane    Set landmark in cam0 (red marker appears)
    Left-click in cam1 pane    Set landmark in cam1 (red marker appears)
    1 / 2 / 3 / 4              Set phase: open / opening / closing / closed
    RIGHT arrow or 'd'         Advance to next frame (saves current annotation if both pts set)
    LEFT arrow or 'a'          Go back one frame
...
Saved alongside cam0 video: <cam0_video>.stereo_annotations.csv
Auto-resumes from this file if it exists.
"""
...
def main() -> None:
    ...
    csv_path = args.cam0_video.with_suffix(args.cam0_video.suffix + ".stereo_annotations.csv")
    ...
    while True:
        ...
        cv2.imshow(WINDOW, _render(frame0, frame1, ann, state, n_total, n_done))
        ...
        if key == ord("q"):
            ...
            state.dirty = True
        elif key in (83, ord("d")) and state.frame_idx + 1 < n_total:  # right
            state.frame_idx += 1
