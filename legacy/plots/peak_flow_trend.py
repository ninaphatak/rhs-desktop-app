"""Plot mean peak flow rate per lap with error bars and a fitted curve.

Quick one-off script — not part of the app.
  python legacy/plots/peak_flow_trend.py [CSV]
"""
...
HARDCODED_CSV = "outputs/Flow-Resistance-90-0-90-Final-Draft.csv"
HARDCODED_LABELS = {
    1: "90",
    2: "80",
    3: "70",
...
def find_flow_peaks(t: np.ndarray, flow: np.ndarray) -> np.ndarray:
...
def main() -> None:
    ...
    lap_nums = [ln for ln in sorted(df["Lap"].unique()) if 1 <= ln <= 10]
    labels: dict[int, str] = {
    ...
    for i, ln in enumerate(lap_nums):
        sub = df[df["Lap"] == ln]
        peaks = find_flow_peaks(sub["Time (s)"].values, sub["Flow Rate (mL/s)"].values)
        if len(peaks) == 0:
    ...
    # Smooth trend line that actually passes through every point.
