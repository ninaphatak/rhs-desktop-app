...
## What This App Does
Unified GUI for: Arduino sensor monitoring (HR, FR, P1, P2), data recording to CSV, data visualization, and camera-based leaflet tracking on a silicone heart valve. Fiducial markers (black dots) on the silicone tricuspid valve are tracked to measure 3D leaflet displacement during simulated cardiac cycles, providing the data needed to characterize valve mechanical behavior and eventually calculate strain.
...
- Tracking is challenging: water reflections, uneven lighting, partially occluded dots — approach still being determined
...
- 🔨 Leaflet tracking — approach in flux; exploring LK optical flow, region-based tracking, and other methods
- 🔨 CameraPanel UI — camera feed display and tracking overlay
...
Tracking fiducial dots on the silicone valve is the core computer vision challenge. The approach is actively being developed and is not yet settled — do not assume any specific method is finalized. Known constraints:

- Automatic global thresholding fails due to water reflections and uneven lighting
- Dots are imperfect circles and may be partially occluded
- Need frame-to-frame continuity across cardiac cycles
- Candidates under evaluation: Lucas-Kanade optical flow, region-based tracking, manual seed + refinement

**Relevant files (subject to change):**
- `src/core/dot_tracker.py` — current tracker implementation
- `src/ui/camera_panel.py` — CameraPanel widget (QGraphicsView)
- `src/ui/graphics_items.py` — overlay graphics items
...
## Available Skills
When using any of the following skills check `.claude/skills/`
- **weekly-progress-summary** — Generates a structured slide-by-slide outline for BIEN 175B weekly progress reports. Trigger phrases: "make my weekly summary", "create progress slides", "generate my weekly report". Reads git history and conversation transcripts to pull content automatically. Output is markdown outline only — no PPTX file generated.
- **update-memory** — Reads recent JSONL conversation transcripts and rewrites `memory/MEMORY.md`. Trigger phrases: "update memory", "refresh memory", "sync memory".
- **manual-dot-selection** — Manual dot selection UI pattern for the camera panel.
- **pyqt-threading** — PyQt6 QThread patterns for real-time I/O (serial, camera).
- **arduino-serial-protocol** — Arduino serial data format and parsing for the RHS.
